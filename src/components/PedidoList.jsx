...
export default function PedidoList() {
  ...
  // La función ahora espera el _id que viene de MongoDB
          ...
          pedidos.map((pedido) => (
            <div key={pedido._id} className={`pedido-card ${pedido.estado === "Listo para entregar" ? 'listo' : 'pendiente'}`}>
              ...
              <div className="pedido-actions">
                <button onClick={() => handleStatusChange(pedido._id, pedido.estado)} className="btn btn-listo">
                  {pedido.estado === "Pendiente" ? "✔️ Marcar como Listo" : "↩️ Marcar como Pendiente"}
                </button>
                <button onClick={() => deletePedido(pedido._id)} className="btn btn-eliminar">
                  🗑️ Eliminar
                </button>
              </div>
            </div>
